...
- **Symlinks attacks**
  ...
  - If there is a check to control if the link is linking to a privileged folder, you can do a double link:
...
- Interesting files:
  ```bash
  find / -type f \( -iname *_hist -o -iname *_history -o -name *.keytab* -o -iname *krb5cc* -o -name *.kt -o -name *.ccache \) -exec ls -l {} \; 2>/dev/null
	```
...
### Network shares
#### From Linux
- [MANSPIDER](https://github.com/blacklanternsecurity/MANSPIDER): looks for files containing `[STRING]`
  ```bash
  manspider.py -t 256 [IP] -u [USER] -p [PWD] -c [STRING]
	```
	- `--sharenames [SHARES]`
	- `-e [EXTENSIONS NO DOT]`
	- `--dirnames`: look into that directory
- `nxc smb --spider IT --content --pattern "passw"`
#### From Windows
- [Snaffler](https://github.com/SnaffCon/Snaffler) `Snaffler.exe -s`
	- `-u users.txt`: searches for references to those users
